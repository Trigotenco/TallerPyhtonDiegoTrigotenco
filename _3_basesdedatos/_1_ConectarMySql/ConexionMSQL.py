...
class ConexionMysql:
    ...
    def Insertar(self, roles, valores)->bool:
        sql = f"INSERT INTO {roles} (nombre_rol) VALUES ( %s)"
...
if __name__ == '__main__':
    conexion=ConexionMysql("127.0.0.1",3306,"almacen","root","password")
    ...
    if conexion.BanConexion:
        if conexion.Insertar("roles",("Empleado",)):
        ...
        if res != None:
            for fila in res:
                print(
                    f"ID{fila[0]},Nombre de Usuario:,{fila[1]}), Correo:,{fila[2]}, Contraseña: {fila[3]}, Id_Rol:{fila[4]}")
        else:
